...
export default () => {
    ...
    const [initialState, setInitialState] = useState({
        current: 1,
        mesCuenta: 1,
        dia: getCurrentDate().substring(8,10),
    ...
    const Mes = ({day}) => {
        return (
            <View 
                ...
                <Text style={{fontSize: isIphone ? 15.5 : 14.5, fontWeight: 'bold', color: '#adadad'}}>{day}</Text>
            </View>
        )
    }
    ...
    const handlePress = (id, dia) => {
        ...
        } else setRange({...range, from: id, until: id, dataFrom: {...dataFrom, day: dia, month: mes, year: año, initial: new Date(año, mes - 1, dia), complete: complete}, dataUntil: {...dataUntil, day: dia, month: mes, year: año, ending: new Date(año, mes - 1, dia), complete: complete}})
    }
    ...
    return(
        <>
            <View style={[tw`h-auto self-stretch justify-center items-center border-[#eaeaea] overflow-hidden`, {borderWidth: 1.8, borderRadius: 2}]}>
                <View style={[tw`h-10 w-[100%] flex-row bg-white border-b border-b-[#eaeaea]`, {borderBottomWidth: 1.8, borderTopStartRadius: 2, borderTopEndRadius: 2}]}>
                    ...
                    <TouchableOpacity disabled={mesCuenta > 1 ? false : true} style={tw`w-12.5 h-[100%] justify-center items-center`} onPress={() => handleChangeMonth('-')}>
                        <IonIcons name={'chevron-left'} size={18} color={mesCuenta > 1 ? '#327BC0' : '#dadada'} />
                    </TouchableOpacity>
                        ...
                        renderItem={({item}) => 
                            ...
                            ?
                                <TouchableOpacity style={tw`flex-1 h-11.5 justify-center items-center`} onPress={() => handlePress(item.id, item.dia)}>
                                    <View style={tw`w-[100%] h-[85%] justify-center items-center ${item.id === from ? 'rounded-l' : 'rounded-none'} ${item.id === until ? 'rounded-r' : 'rounded-none'} bg-[${(((item.fecha >= dataFrom.initial) && (item.fecha <= dataUntil.ending))) ? (item.id === from || item.id === until) ? '#327BC0' : 'rgba(50,131,197,.1)' : '#fff'}]`}>
                                        <View style={[tw`w-[100%] h-9 justify-center items-center`]}>
                                            <View style={[tw`w-7 h-7 justify-center items-center rounded-3xl ios:pl-px`]}>
                                                <Text style={[tw`text-[${(((item.fecha >= dataFrom.initial) && (item.fecha <= dataUntil.ending)) || (item.id === from || item.id === until) || item.today) ? (item.id === from || item.id === until) ? '#fff' : '#327BC0' : '#383838'}] text-xs font-${(((item.fecha >= dataFrom.initial) && (item.fecha <= dataUntil.ending)) || (item.id === from || item.id === until) || item.today) ? 'bold' : 'normal'}`, {fontSize: 12}]}>{item.dia}</Text>
                                                {
                                                    item.today
                                                    &&
                                                        <View style={{height: 5, width: 5, borderRadius: 5, backgroundColor: item.today ? ((item.id === from) || (item.id === until)) ? '#fff' : '#327BC0' : 'transparent', position: 'absolute', bottom: 1}}/>
                                                }
                                            </View>
                                        </View>
                                    </View>
            ...
            <View style={tw`h-auto self-stretch mt-2.5`}>
                <TextInput
                    editable={false}
                    style={{height: 45, alignSelf: 'stretch', backgroundColor: '#fff', fontSize: 14, color: '#adadad', fontWeight: 'normal', borderWidth: 1, borderColor: '#eaeaea', textAlign: 'center'}}
                    value={origin}
                />
            </View>
        </>
